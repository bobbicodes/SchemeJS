...
export function createInstance(schemeOpts = {}) {
  ...
  function Atom(name) {
    ...
    if (parts.length > 1) {
      for (let i = 0, lng = parts.length; i < lng; ++i) {
        let part = parts[i];
  ...
  function checkUndefinedInScope(symbol, scope) {
  ...
  function apply(fn, args, scope = this) {
    ...
    if (TRACE_INTERPRETER) {
      fName = namedObjects.get(fn) ?? fn.name;
      console.log("APPLY (API)", fName, ...argv);
    }
    let result = fn.apply(scope, argv);
    if (TRACE_INTERPRETER)
      console.log("RETURNS", fName, result);
  ...
  // Equality
  //
  // Because stack depths are finite, equal must have bounds on its recursion.
  // The bounds are large and user-optionble, but they are finite. If the bounds
  // are exceeded, deep_eq returns "undefined," which is "falsey" but distinguishable
  // from "false."
  //
  ...
  function equal(a, b, maxDepth = MAX_INTEGER, maxLength = MAX_INTEGER, report = {}) {
    let seen = new Map();
    if (a === b) return true;
    ...
    function deep_eq(a, b, depth, length) {
      ...
      // Check recursion; we already know the objects aren't ===
      if (seen.get(a) || seen.get(b)) {
        report.a = a, report.b = b;
        report.valuesDiffer = true;
        return false;
      }
      seen.set(a, true), seen.set(b, true);
